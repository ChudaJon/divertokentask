                                    ...
                                    <div>
                                        ...
                                        <Grid.Container spacing={2}>
                                            <Grid.Item xs={GridSize.size(2)}>
                                                <Button variant=Button.Variant.contained color="primary" onClick={0->handleModal} >{string("Verify")}</Button>
                                            </Grid.Item>
                                            <Grid.Item xs={GridSize.size(2)}>
                                                <Button variant=Button.Variant.contained color="secondary" onClick={1->handleModal} >{string("Decline")}</Button>
                                            </Grid.Item>
                                        </Grid.Container>
                                        </div>
                                        <Modal _open={openModal} onClose={handleClose}>
                                            <div style=(ReactDOM.Style.make(~position="absolute", ~backgroundColor="#FFFFFF", ~top="50%", 
                                            ~left="50%", ~transform="translate(-50%, -50%)", ~width="400", ~borderRadius="3px 3px", ()))>
                                            <Box p={4}>
                                                    ...
                                                    {showVerify == false ? 
                                                        <div>
                                                            <Typography id="modal-modal-description">
                                                                {string("Do you want to decline " ++ task.content ++ "? If everyone who voted has verified this task, " ++ user.displayName ++ " will receive "
                                                        ...
                                                        <div>
                                                            <Typography id="modal-modal-description">
                                                                {string("Do you want to verify " ++ task.content ++ "? If everyone who voted has verified this task, " ++ user.displayName ++ " will receive "
                                                                ++ string_of_int(task.vote))}
                                                                { task.vote != 1 ? {string(" tokens")} : {string(" token")} }
                                                            </Typography>
                                                            <div style=(ReactDOM.Style.make(~padding="20px 0px 0px 0px", ()))>
