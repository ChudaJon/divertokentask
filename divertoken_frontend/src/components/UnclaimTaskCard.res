              ...
              <div style=(ReactDOM.Style.make(~margin="auto", ()))>
                <Grid.Item xs={GridSize.size(4)}>
                  { Js.Dict.get(task.voted, user.id) == Some(0) || Js.Dict.get(task.voted, user.id) == None ? 
                    <Button color="secondary" variant=Button.Variant.contained onClick={_ => vote(user, task)} >
                      {string("Vote")}
                    </Button>
                  : <Button color="secondary" variant=Button.Variant.contained onClick={_ => handleModal(user, task)} >
                      {string("Unvote")}
                    </Button> }
                      ...
                      <Box p={4}>
                        ...
                        <div style=(ReactDOM.Style.make(~padding="20px 0px 30px 0px", ()))>
                          <Typography id="modal-modal-description">
                            {string("This task has only 1 vote left, if you unvote it, it will be automatically deleted")}
                          </Typography>
                        </div>
                        <Grid.Container spacing={2}>
                          <Grid.Item xs={GridSize.size(6)}>
                            <Button color="primary" variant=Button.Variant.contained onClick={_ => handleDelete(user, task)} >
                              {string("Delete Task")}
                            </Button>
                          </Grid.Item>
                          <Grid.Item xs={GridSize.size(6)}>
